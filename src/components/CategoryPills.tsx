...
export function CategoryPills({categories, selectedCategory, onSelect}:CategoryPillProps){
    ...
    const [isRigthVisible, setIsRightVisible]= useState(true)
    ...
    return (
        <div className="overflow-x-hidden relative">
            <div ref={containerRef} className="flex whitespace-nowrap gap-3 transition-transform w-[max-content]"
            ...
            {isLeftVisible && (
             ...
             {isRigthVisible && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 bg-gradient-to-1 from-white from-50% to-transparent w-24 h-full">
                    <Button variant='ghost' size='icon'
                    className="h-full aspect-square w-auto p-1.5" 
                    onClick={()=>{
