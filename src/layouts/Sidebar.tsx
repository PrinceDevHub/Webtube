import { ChevronDown, ChevronUp, Clapperboard, Clock, Film, Flame, Gamepad2, Home, Library, Lightbulb, ListVideo, Music2, Newspaper, PlaySquare, Podcast, Radio, Repeat, Shirt, ShoppingBag, Trophy } from "lucide-react";
import { Children, useState, type ElementType, type Key, type ReactNode } from "react";
    ...
    <>
      ...
      <aside className={`w-56 lg:sticky absolute top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 px-2 hidden ${isLargeOpen ? 'lg:flex' : 'lg:hidden'} ${isSmallOpen ? 'flex z-[999] bg-white max-h-screen' : 'hidden'}`}>
        ...
        <LargeSidebarSection>
          ...
          {playlists.map((playlist: { id: Key | null | undefined; name: string }) => (
            <LargeSidebarItem
              key={playlist.id}
        ...
        <LargeSidebarSection title="Subscriptions">
          {/* Dynamically generated subscriptions */}
...
function LargeSidebarItem({ IconOrImgUrl, title, url, isActive = false }: LargeSidebarItemProps) {
  return (
    <a
      href={url}
      className={twMerge(buttonStyles({ variant: 'ghost' }), `w-full flex items-center rounded-lg gap-4 p-3 ${isActive ? 'font-bold bg-neutral-100 hover:bg-secondary' : ''}`)}
      role="link"
      aria-label={title}
    >
